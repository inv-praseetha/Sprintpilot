...
3. Weekend Exclusion: Saturdays and Sundays are non-working days. Under NO circumstances should `planned_start_date` or `planned_end_date` be set to a Saturday or Sunday.
4. Working Days Calculation: Calculate `working_days` as the number of weekdays (Monday through Friday) between `planned_start_date` and `planned_end_date` (inclusive).
