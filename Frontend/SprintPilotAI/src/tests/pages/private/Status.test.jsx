...
describe('Status Page Component', () => {
  ...
  it('triggers loadMoreTasks when container is scrolled to bottom', async () => {
    const paginatedData = {
      ...mockStatusData,
      overdue: {
        ...mockStatusData.overdue,
        has_more: true,
        total_count: 6
      }
    };
    apiClient.get.mockResolvedValueOnce({ data: paginatedData });
    ...
    const overdueContainer = screen.getByText('Overdue').closest('.flex-col').querySelector('.overflow-y-auto');
    expect(overdueContainer).toBeInTheDocument();

    Object.defineProperty(overdueContainer, 'scrollHeight', { value: 1000, configurable: true });
